...
> verified `build/tower-bloxx-full.bin` at offset `0x0` only when the
...
Firmware validation uses a fresh temporary build directory and an isolated `sdkconfig` generated from the tracked defaults. It does not consume or overwrite a developer's root `sdkconfig`, and it copies only the verified merged image to `build/tower-bloxx-full.bin`. The gate also enforces the [protected Flash layout](protected-flash-layout.md): the protected `cardid` address, application size, partition-table MD5, and absence of device-specific identity data.
...
Never upload the app-only `build/tower-bloxx.bin` to the community. Only
the validated `build/tower-bloxx-full.bin` contains the complete checked
firmware layout.
