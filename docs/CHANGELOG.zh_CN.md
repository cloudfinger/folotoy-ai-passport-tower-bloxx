...
- 将固件项目和完整 BIN 更名为 `tower-bloxx`，同步修改内嵌的 ESP-IDF 应用标识、CI 产物和构建校验，让固件读取工具显示游戏名称。

- 在原有黄昏暗色主题之外新增可选的日光浅色主题，封面、游戏背景、楼层、城市地图和文字对比度同步切换；标题菜单中的选择会保存，重启后继续使用。

- 新增原创循环芯片音乐，以及菜单移动、放下楼层、落点、Perfect、失误、胜利、失败和城市放置的短音效。音频由独立任务持续播放，避免阻塞按键与堆楼动画。
- 延长并增强关键游戏音效，以不同的上行／下行乐句区分反馈；重要音效播放时会平滑、短暂地降低背景音乐音量。

- 为实时堆叠与城市地图画面加入同风格的黄昏城市背景。游戏背景中央保留简洁区域，让移动中的方形楼层清楚可见；标题与结算页继续使用原封面插画。

- 将 Tower Bloxx 分支改为独立游戏：开机直接显示原创像素城市封面与短暂的载入进度条，随后进入带插画的城市模式／快速游戏标题菜单；长按 OK 返回游戏标题。此固件镜像不再包含原先的硬件演示启动菜单与不用的演示服务。

- 新增适用于 Passport 240×320 屏幕的离线三键 Tower Bloxx 游戏，包含按时机堆叠楼层、快速游戏、城市放置、经过主机测试的玩法规则，以及适配 LVGL 内存池的画面切换。楼层现采用正方形的 4×4 窗格立面，固件各页面统一改用砖红、琥珀、米色和深色施工状态栏组成的暖色像素城市风格。
